export default function generateUserStatus(user) {
  ...
  const statusData = [
    {
      ...
      text: () => user.cash,
      onClick: coinIconOnClick,
    },
    {
      sprite: { name: "controller", opt: {} },
      text: () => user.FP,
      onClick: facilityIconOnClick,
    },
    {
      sprite: { name: "config", opt: {} },
      text: () => user.MP,
      onClick: gearIconOnClick,
    },
    {
    ...
    { sprite: { name: "onion", opt: {} }, text: () => user.enrolled },
  ];
  const HEIGHT =
    (statusData.length + 1) * GAP + statusData.length * ICON_HEIGHT;
  ...
  statusData.forEach((data, i) => {
    userStatus.add([
      sprite(
        data.sprite.name,
        Object.assign(data.sprite.opt, { height: ICON_HEIGHT }),
      ),
    ...
    userStatus.add([
      text(data.text() !== null ? `${convertUnit(data.text())}` : "", {
        size: TEXT_SIZE,
      }),
      pos(
